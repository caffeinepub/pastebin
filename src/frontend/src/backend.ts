...
import type { Paste as _Paste, UserProfile as _UserProfile, UserRole as _UserRole } from "./declarations/backend.did.d.ts";
export class Backend implements backendInterface {
    ...
    async createPaste(arg0: string, arg1: string): Promise<PasteId> {
        if (this.processError) {
            try {
    ...
    async getPaste(arg0: PasteId): Promise<Paste | null> {
        if (this.processError) {
            try {
                const result = await this.actor.getPaste(arg0);
    ...
    async listPastes(): Promise<Array<PasteSummary>> {
        if (this.processError) {
            try {
                const result = await this.actor.listPastes();
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
...
function from_candid_opt_n6(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: [] | [_Paste]): Paste | null {
    return value.length === 0 ? null : value[0];
}
